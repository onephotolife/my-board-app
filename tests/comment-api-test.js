...
async function testCommentAPI() {
  try {
    ...
    const posts = postsData.data || postsData; // data フィールドがある場合とない場合に対応
    ...
    if (httpCode === '404') {
      ...
      // Dynamic Routeの問題の可能性を確認
      console.log('\n## 診断: Dynamic Route問題の確認');
      CommentAPIDebugLogger.log('CHECK_DYNAMIC_ROUTE');
      
      // ファイル存在確認
      const checkFileCommand = `ls -la src/app/api/posts/\\[id\\]/comments/route.ts 2>&1`;
      const { stdout: fileCheck } = await execPromise(checkFileCommand);
      
      CommentAPIDebugLogger.log('FILE_EXISTS', {
        exists: !fileCheck.includes('No such file'),
        fileInfo: fileCheck.trim()
      });
      
      return { 
        getStatus: '404 - NOT_IMPLEMENTED',
        postStatus: 'SKIP',
        diagnosis: 'Dynamic Route問題の可能性'
      };
    }
    
    // Step 4: コメント投稿テスト（POST）
    console.log('\n## Step 4: コメント投稿テスト');
    CommentAPIDebugLogger.log('POST_COMMENT', { postId });
    
    // アプリケーション独自のCSRFトークン取得
    const { stdout: appCsrfResponse } = await execPromise(`curl -s -b /tmp/auth-cookies.txt "${BASE_URL}/api/csrf"`);
    const appCsrfData = JSON.parse(appCsrfResponse);
    
    CommentAPIDebugLogger.log('APP_CSRF_TOKEN', {
      hasToken: !!appCsrfData.token,
      header: appCsrfData.header
    });
    ...
    const postCommentCommand = `curl -s -X POST \\
      ...
      -H "X-CSRF-Token: ${appCsrfData.token}" \\
    ...
    // クリーンアップ
    await execPromise('rm -f /tmp/auth-cookies.txt').catch(() => {});
  }
}
