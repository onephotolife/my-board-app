...
export async function GET(request: NextRequest) {
  try {
    ...
    console.warn('🔍 トークンタイプ:', tokenType);

    console.warn('🔍 メール確認トークン:', token);
    ...
    if (!userWithToken) {
      ...
      console.warn('🔍 データベース内の既存トークン（最大5件）:', 
        similarTokens.map(u => ({
          email: u.email,
    ...
    console.warn('✅ メール確認完了:', user.email);
    console.warn('📄 更新後のユーザー情報:', {
      email: user.email,
