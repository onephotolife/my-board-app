...
export async function POST(req: NextRequest) {
  try {
    ...
    // リクエストボディの取得（空ボディは {} として扱う）
    let body: Record<string, unknown> = {};
    try {
      // 空ボディの場合 req.json() は例外を投げるため、catch で {} にフォールバック
      body = await req.json();
    } catch {
      body = {};
    }
    ...
    if (
      ...
      // 特定の通知を既読にする
      const validIds = body.notificationIds.filter((id: string) => /^[0-9a-fA-F]{24}$/.test(id));
