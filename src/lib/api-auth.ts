...
export async function requireEmailVerifiedSession() {
  ...
  try {
    ...
    if (!session?.user?.email) {
      ...
      throw new ApiAuthError('UNAUTHORIZED', 401, '認証が必要です');
    }

    // メール確認チェック（会員制掲示板として必須）
    if (!session.user.emailVerified) {
      console.warn('📧 [API Security] メール未確認のためアクセス拒否');
      throw new ApiAuthError('EMAIL_NOT_VERIFIED', 403, 'メール確認が必要です');
    }
    ...
    throw new ApiAuthError('UNAUTHORIZED', 500, '認証処理でエラーが発生しました');
  }
}
...
export function withApiAuth(
  handler: (req: NextRequest, session: unknown) => Promise<NextResponse>,
  ...
  return async (req: NextRequest): Promise<NextResponse> => {
    try {
      ...
      return createApiErrorResponse(
        new ApiAuthError('UNAUTHORIZED', 500, '予期しないエラーが発生しました')
      );
    }
  };
}
 ...
 */
export function hasAdminPermission(session: unknown): boolean {
 ...
 */
export function hasModeratorPermission(session: unknown): boolean {
  const s = session as { user?: { role?: string } } | null | undefined;
  const isModerator = s?.user?.role === 'moderator' || hasAdminPermission(session);
